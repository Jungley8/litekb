...
from app.models import (
    ...
    Conversation, Message, GraphEntity, GraphRelation,
    Organization, OrganizationMember, APIKey, RevokedToken,
    KBDocument, Plugin, UserSetting, AuditLog, Share,
    ImportJob, ExportJob, OrgInvitation, KBActivity
)
...
class ORMStore:
    ...
    def list_docs(self, kb_id: str = None, skip: int = 0, limit: int = 100) -> List[Document]:
        with self.get_session() as session:
    ...
    def list_chunks(self, kb_id: str = None, doc_id: str = None, limit: int = 100) -> List[DocumentChunk]:
        with self.get_session() as session:
    ...
    def list_conversations(self, kb_id: str = None, user_id: str = None, limit: int = 50) -> List[Conversation]:
        with self.get_session() as session:
    ...
    def list_entities(self, kb_id: str = None, entity_type: str = None, limit: int = 100) -> List[GraphEntity]:
        with self.get_session() as session:
    ...
    def list_relations(self, kb_id: str = None, source_id: str = None, limit: int = 100) -> List[GraphRelation]:
        with self.get_session() as session:
    ...
    def get_share_by_token(self, token: str) -> Optional[Share]:
        with self.get_session() as session:
            return session.query(Share).filter(
                Share.token == token, Share.is_active == True
            ).first()
    ...
    def list_shares(self, resource_type: str = None, resource_id: str = None) -> List[Share]:
        with self.get_session() as session:
    ...
    def is_token_revoked(self, token_hash: str) -> bool:
        with self.get_session() as session:
            revoked = session.query(RevokedToken).filter(
