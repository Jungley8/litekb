...
async def login(user: UserLogin):
    ...
    if not existing or not verify_password(
        ...
        (
            ...
            if hasattr(existing, "hashed_password")
            else existing.get("hashed_password", "")
        ),
    ):
        raise HTTPException(status_code=400, detail="Incorrect username or password")

    access_token = create_access_token(
        data={"sub": existing.id if hasattr(existing, "id") else existing["id"]}
    )
...
async def create_kb(
    kb: KnowledgeBaseCreate, current_user: User = Depends(get_current_user)
):
    """创建知识库"""
    kb_data = {
        "name": kb.name,
        ...
        "created_by": (
            current_user.id if hasattr(current_user, "id") else current_user["id"]
        ),
    }
...
async def create_doc(
    kb_id: str, doc: DocumentCreate, current_user: User = Depends(get_current_user)
):
    """创建文档"""
    kb = db.get_kb(kb_id)
    if not kb:
...
async def delete_document(
    kb_id: str, doc_id: str, current_user: User = Depends(get_current_user)
):
    """删除文档"""
...
async def search_kb(
    kb_id: str, request: SearchRequest, current_user: User = Depends(get_current_user)
):
    """知识库内搜索"""
    kb = db.get_kb(kb_id)
    if not kb:
...
async def chat_with_kb(
    kb_id: str, chat: ChatRequest, current_user: User = Depends(get_current_user)
):
    """RAG 对话"""
    kb = db.get_kb(kb_id)
    if not kb:
    ...
    response = await rag_engine.query(
        kb_id=kb_id, question=chat.message, mode=chat.mode, history=chat.history or []
    )
...
async def build_graph(
    kb_id: str, rebuild: bool = False, current_user: User = Depends(get_current_user)
):
    """构建知识图谱"""
    kb = db.get_kb(kb_id)
    if not kb:
