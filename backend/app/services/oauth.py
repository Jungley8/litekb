...
class OAuthService:
    ...
    def get_authorization_url(self, provider: OAuthProvider, state: str = None) -> str:
        """获取授权 URL"""
    ...
    def _normalize_user_info(
        self, provider: OAuthProvider, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """标准化用户信息"""
        normalizers = {
    ...
    def link_account(
        self, user_id: str, provider: OAuthProvider, provider_id: str
    ) -> bool:
        """绑定第三方账号"""
        # TODO: 保存到数据库
        logger.info(
            f"Linked account: user={user_id}, provider={provider.value}, id={provider_id}"
        )
        return True
...
class SSOService:
    ...
    async def handle_sso_callback(
        ...
        # 2. 获取用户信息
        user_info = await self.oauth.get_user_info(provider, tokens.get("access_token"))
...
class SAMLAuth:
    ...
    def configure(self, idp_metadata_url: str, entity_id: str, acs_url: str):
        """配置 SAML"""
        try:
