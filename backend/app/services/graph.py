...
class KnowledgeGraphService:
    ...
    async def build_graph(
        ...
        try:
            ...
            for entity in entities:
                existing = session.query(GraphEntity).filter(
            ...
            for relation in relations:
                ...
                if source_id and target_id:
                    existing = session.query(GraphRelation).filter(
    ...
    def get_graph(self, kb_id: str) -> Dict[str, Any]:
        ...
        session = get_session()
        try:
            entities = session.query(GraphEntity).filter(
                GraphEntity.kb_id == kb_id
            ).all()
            
            relations = session.query(GraphRelation).filter(
    ...
    def search_entities(self, kb_id: str, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            entities = session.query(GraphEntity).filter(
                GraphEntity.kb_id == kb_id,
                GraphEntity.entity_name.contains(query)
            ).limit(20).all()
    ...
    def get_entity_relations(self, kb_id: str, entity_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取出边
            outgoing = session.query(GraphRelation).filter(
                GraphRelation.source_id == entity_id
            ).all()
            
            # 获取入边
            incoming = session.query(GraphRelation).filter(
